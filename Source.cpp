...
int mint(int a,int b){
	...
	double t1 =10/countc[a];
	double t2 = 10/countc[b];
...
int main() {
	...
	Mat img = imread("seg_test.jpg", CV_LOAD_IMAGE_COLOR);
	Mat out= imread("seg_test.jpg");
	imshow("ori", out);
	...
	for (int i = 0;i < cnt;i++) {
		...
		if (u == v) {
			...
			if (u != v && tmp.w <= thresholds[u] && tmp.w <= thresholds[v]) {
				if (Rank[u] < Rank[v]) {
					f[u] = v;
					maxc[v] = tmp.w;
					countc[v] += countc[u];
					thresholds[u] = tmp.w + 10.0 / countc[v];
					countc[u] = 0;
				}
					...
					maxc[u] = tmp.w;
					countc[u] += countc[v];
					thresholds[v] = tmp.w + 10.0 / countc[u];
					countc[v] = 0;
				}
				else {
					f[u] = v;
